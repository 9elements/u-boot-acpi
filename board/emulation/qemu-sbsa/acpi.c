...
int board_init(void)
{
	...
	uclass_foreach_dev_probe(UCLASS_CPU, cpu) {
		...
		ret = smc_get_mpidr(dev_seq(cpu), &mpidr);
		if (ret) {
		...
		cpu_plat->gicc_cpu_if_num = dev_seq(cpu);
		cpu_plat->gicc_perf_gsiv = SBSAQEMU_MADT_GIC_PMU_IRQ;
		cpu_plat->gicc_phys_base = 0;
		cpu_plat->gicc_gicv = SBSAQEMU_MADT_GIC_VBASE;
		cpu_plat->gicc_gich = SBSAQEMU_MADT_GIC_HBASE;
		cpu_plat->gicc_vgic_maint_irq = 25;
		cpu_plat->gicc_gicr_base = 0;
		cpu_plat->gicc_mpidr = mpidr;
		cpu_plat->gicc_efficiency = 0;
	}
